...
class CountryValidator extends \Magento\Payment\Gateway\Validator\AbstractValidator
{
    ...
    public function validate(array $validationSubject)
    {
        ...
        if($country == 'EC' && in_array($this->storeManager->getStore()->getBaseCurrencyCode(), $available_currencies)){
            $isValid = true;
        }

        if (!$isValid) {
            $this->_logger->info('Country is not EC or the store base currency is not in the available currencies');
        }
