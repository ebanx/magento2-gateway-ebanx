<?php
namespace DigitalHub\Ebanx\Test\Gateway\Request\Ecuador\SafetyPay;
...
use DigitalHub\Ebanx\Gateway\Request\Ecuador\SafetyPay\PaymentDataBuilder;
...
class PaymentDataBuilderTest extends \PHPUnit\Framework\TestCase
{
    public function testBuildCash()
    {
        ...
        $paymentModelMock->expects($this->any())
            ...
            ->willReturn([
                DataAssignObserver::SAFETYPAY_TYPE => 'cash'
            ]);
    ...
    public function testBuildOnline()
    {
        $objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager($this);

        $storeId = 1;
        $days = 2;
        $type = 'safetypayonline';
        $dueDate = new \DateTime(date('Y-m-d', strtotime('now +' . $days . 'days')) . ' 00:00:00');
        $amountTotal = 123.45;
        ...
        $paymentModelMock->expects($this->any())
            ->method('getAdditionalInformation')
            ->willReturn([
                DataAssignObserver::SAFETYPAY_TYPE => 'online'
            ]);
